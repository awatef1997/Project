...
if(isset($_POST['submit'])){
  ...
  $password2 = $connection->real_escape_string($_POST['password_2']);
  $query = $connection->query("SELECT * FROM user WHERE name = '$username'");
  if(empty($username)OR empty($password1)OR empty($email) OR empty($password2)){
    $output = "please fill in all feild";
  }elseif($query->num_rows != 0){
    $output = "that is taken";
  }elseif ($password2 != $password1) {
    // code...
    $output = "the password doesnt match";
    ...
    // record
    $insert = $connection->query("INSERT INTO user(name, password,email) VALUES ('$username','$password1','$email')");
        if($insert != TRUE){
                ...
                <div class="card-body">
                  ...
                  <form class="form-signin">
                    <div class="form-label-group">
                      <input type="text" id="inputtext" class="form-control" placeholder="text" name="username" required autofocus>
